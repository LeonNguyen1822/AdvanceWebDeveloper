...
<form action="member_search.php" method = "post">
    <fieldset>
        <legend style="font-weight:bold">Search for member(s):</legend>
...
<?php
    ...
    if (!empty($_POST["mLast"]))
    {
        //Connect to the DB
        $dbConnect = mysqli_connect($host, $user, $pswd, $dbnm)
            or die ("<p style=\"color:red\">Unable to connect to the database server.</p>" . "<p>Error code " . mysqli_connect_errno() . ": " . mysqli_connect_error() . "</p>");
        ...
        if (mysqli_num_rows($results) <> 0) {
            //Query the search
            $query = "SELECT member_id, fname, lname, email FROM vipmembers WHERE lname = ". "'" . $_POST["mLast"] . "';";
            $results = mysqli_query($dbConnect, $query);
            if (mysqli_num_rows($results) <> 0)
            {
                ...
                while ($row) {
                    ...
                    echo "<td style=\"border: 1px solid black\">$row[3]</td></tr>";
                    $row = mysqli_fetch_row($results);
                }
                echo "</table>";
            } else {
                echo "<p style=\"color:red\">No records matching the lastname " . "<b>'" . $_POST["mLast"] . "'</b> found!</p>";
            }
        } else {
