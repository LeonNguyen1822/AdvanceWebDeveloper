...
<body class="bg_image">
  ...
  //session check that ensures a user is logged in before allowing access to certain pages or functionality
  ...
  {
    global $conn, $numOfFriends, $userId, $friends, $myfriends;

    // Insert a new row into myfriends table
  ...
  // calculates the total number of accounts (names) that are not friends of the logged-in user
  ...
  // Retrieve friends name for the current page
  ...
  <div class="content">
    ...
    <div class='addfriendtable'>
      ...
      if (mysqli_num_rows($result) > 0) {
        echo "<table class='table table-bordered table-primary table-striped'>";
        ...
        while ($row = mysqli_fetch_assoc($result)) {
          ...
          $sql2 = "SELECT friend_id, COUNT(*) AS mutual_friend_count
                  ...
                  AND f.friend_id IN (
                    ...
                    UNION SELECT friend_id2 FROM $myfriends WHERE friend_id1 = $userId
                  )";
          ...
          echo "<td>
                  <form method='post' action='friendadd.php'>
                    ...
                    <input class='btn btn-outline-success' type='submit' name='addfriend' value='Add as friend'>
                  </form>
                </td>";
          echo "</tr>";
          echo "</tbody>";
